...
export class StockService {
  ...
  getStockQuantityByProductId(id: number): Observable<number> {
    const params = new HttpParams().set("id", id.toString());
    return this.httpClient.get<number>(environment.getApiUrl + `/stocks/getbyproductid`, {
      params,
    });
  }
  ...
  addStock(stock: Stock): Observable<any> {
    ...
    return this.httpClient.post(environment.getApiUrl + "/stocks/", stock, {
      responseType: "text",
    });
  }

  updateStock(stock: Stock): Observable<any> {
    var result = this.httpClient.put(environment.getApiUrl + `/stocks/`, stock, {
      responseType: "text",
    });
    return result;
  }
